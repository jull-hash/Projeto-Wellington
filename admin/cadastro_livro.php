    ...
    if (!in_array($imagem_livro["type"], $tipospermitidos)){
        ...
        if (empty($error)){
            if (!empty($_POST["id_livro"])) {
                ...
                $sql = "INSERT INTO livro (nome_livro, autor, publicado, genero, preco_livro, imagem_livro, descricao_livro, quantidade_livro) VALUES ('$nome_livro','$autor','$publicado','$genero','$preco_livro','$strimagem_livro','$descricao_livro','$quantidade_livro')";
            }
...
<div class="p-6 flex-1">
  ...
  <main class="main-content">
    <div class="card-formulario">
      ...
      <?php if ($editando): ?>
        <input type="hidden" name="id_livro" value="<?= $editando["id_livro"] ?>">
      <?php endif; ?>
        ...
        <div class="grid-form">
          <div class="grupo-campo linha-completa">
            <label>Título</label>
            <input type="text" value="<?= $editando["nome_livro"] ?? "" ?>" name="nome_livro" placeholder="Ex: O Senhor dos Anéis" required />
          </div>
          <div class="grupo-campo">
            <label>Autor</label>
            <input type="text" value="<?= $editando["autor"] ?? "" ?>" name="autor" placeholder="Ex: J.R.R. Tolkien" required />
          </div>
          <div class="grupo-campo">
            <label>publicado</label>
            <input type="text" value="<?= $editando["publicado"] ?? "" ?>" name="publicado" placeholder="Ex: 29 de julho de 1954" required />
          </div>
          <div class="grupo-campo">
            <label>genero</label>
            <select name="genero">
          ...
          <div class="grupo-campo">
            ...
            <input type="text" value="<?= $editando["descricao_livro"] ?? "" ?>" name="descricao_livro" placeholder="Ex: A história acompanha ..." required />
          </div>
          <div class="grupo-campo">
            <label>preço</label>
            <input type="number" value="<?= $editando["preco_livro"] ?? "" ?>" name="preco_livro" min="0" step="0.01" placeholder="Ex: 0.00" required />
          </div>
          <div class="grupo-campo">
            <label>Quantidade Disponível</label>
            <input type="number" value="<?= $editando["quantidade_livro"] ?? "" ?>" name="quantidade_livro" placeholder="Ex: 9" min="0" required />
          </div>
        </div>

        <div class="form-footer" style="margin-top: 2rem;">
          <a class="link-rodape" href="listar_livros.php">← Voltar ao Catálogo</a>
          <button type="submit" class="btn-primario"><?= $editando ? "Salvar Alterações" : "Cadastrar Produto" ?></button>
        </div>
      </form>
    </div>
  </main>
